...
* **POST /auth/entrar** — Recebe `email` e `senha`, retorna token JWT e dados básicos do usuário.
* **POST /auth/renovar** — Recebe refresh token, devolve novo token JWT.
* **POST /auth/sair** — Invalida o refresh token da sessão atual.

## Métricas e relatatórios
* **GET /admin/metricas/visao-geral** — Retorna faturamento, número de pedidos, novos clientes e ticket médio no período atual.
* **GET /admin/metricas/tendencia-vendas?de=AAAA-MM-DD&ate=AAAA-MM-DD** — Séries temporais para vendas e novos clientes (dados para o gráfico de linhas).
* **GET /admin/metricas/operacoes** — Percentuais de pedidos em rota, separação e aguardando pagamento.

## Produtos
* **GET /produtos?destaque=true&periodo=semana|mes** — Lista produtos em destaque (receita, status, SKU).
* **GET /produtos/:id** — Detalhe completo de um produto.
* **PATCH /produtos/:id** — Atualiza estoque, preço ou status de visibilidade.

## Pedidos
* **GET /pedidos?status=...&pagina=...** — Lista paginada de pedidos recentes, com status e valores.
* **GET /pedidos/:id** — Detalhe do pedido (itens, endereço, linha do tempo de entrega).
* **POST /pedidos** — Cria pedido manual (ação rápida do painel), recebendo itens, cliente e endereço.
* **PATCH /pedidos/:id/status** — Atualiza status logístico (separando, em rota, entregue etc.).

## Clientes
* **GET /clientes/:id** — Dados do cliente e histórico de pedidos.
* **POST /clientes** — Cria cliente quando incluído manualmente.

## Campanhas e alertas
* **POST /campanhas** — Cria campanha promocional (ação rápida "Criar campanha").
* **POST /alertas** — Registra incidentes ou alertas operacionais (ação rápida "Alertas" / "Relatar incidente").

## Webhooks (opcional)
* **POST /webhooks/pagamentos** — Recebe confirmações de pagamento para atualizar status dos pedidos.
* **POST /webhooks/entregas** — Atualiza eventos de entrega (coleta, rota, entregue).
